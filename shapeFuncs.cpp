...
double distanceBetween(Point p, Point q) {
  return sqrt(pow(q.x-p.x,2)+pow(q.y-p.y,2)); // @@@ STUB
...
void initPoint(struct Point *p, double xVal, double yVal) {
  p->x = xVal;
  p->y = yVal; //@@@ for a void function, the stub is just a bare return that does nothing
}
...
bool boxesApproxEqual(Box b1, Box b2, double tolerance) {
  ...
  if(approxEqual(b1.width,b2.width,tolerance) == true && approxEqual(b1.height,b2.height,tolerance) == true && pointsApproxEqual(b1.ul,b2.ul,tolerance) == true){
	  return true;
  }else{
	  return false;
  }
...
double areaOfBox(Box b) {
  return b.height * b.width;  /* stub---make sure all tests fail initially */
